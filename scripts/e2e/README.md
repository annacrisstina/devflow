...
**Requirements:** dev infrastructure running (`docker compose up -d`),
`pnpm install && pnpm build` done (spawned apps import the packages'
compiled output — the harness preflights this), ports 3196–3199 free,
`~/.local/bin` on PATH (pnpm). **Side effects** (cleaned up
