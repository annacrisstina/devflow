...
## Session of 2026-07-19 (evening) — Milestone 6 begun, killed by a Docker crash, resumed from repository state

M5 merged as **PR #10**; M6 opened on `feat/self-host-release`. The session's first two components landed and were committed (worker health server + Prometheus metrics on both processes, ADR-0021; the scripted e2e harness promoted into the repo as `pnpm e2e`). Mid-way through the third component — the ADR-0020 containerized self-hosting path — **the Docker daemon crashed and took the session with it**. Costs of the crash, discovered on resume: the M6 architecture review existed only in that chat and is **lost** (the decisions that shipped survive in ADR-0020/0021; the remaining-scope design must be re-produced and founder-approved before further components — recorded as a process gap in the handoff), and the crash left a mid-word edit in a `webhooks.ts` comment plus deeper damage below.

**The resume (this entry) was the memory system's second in-flight-milestone test, passed:** branch state + committed ADR-0021's forward reference to ADR-0020 + the uncommitted Dockerfiles/compose diff located the interruption point precisely; the two committed components were not re-verified or touched.

**The Docker forensics, worth remembering:** recovery had corrupted the image store — the pgvector `pg17` layer chain held 0-byte files (`postgres` itself, `gosu`, the entrypoints) while `docker ps` looked healthy. Diagnosis sequence: fresh never-seen images extracted perfectly (so extraction was fine _now_); content-store blobs re-hashed clean (so the corruption sat in the unpacked snapshots + image metadata — `docker images` showed pgvector "created 1970, 14.7 kB"); `rmi` + re-pull cycles kept resurrecting the corrupt chain (stale 2025 containerd leases; the metadata survives deletion). Surgical containerd repair was out of reach (permissions), so the fix became a sidestep: **pull `0.8.0-pg17` (different layer chain, extracts clean) and locally retag it as `pg17`** — zero repo changes. The `postgres-data` volume had also been wiped; migrations 0000–0004 were re-applied to a fresh cluster. Lessons recorded: _a healthy daemon does not imply a healthy image store — stat a binary inside the image before trusting it_, and _corrupt extracted layers are keyed by chain ID; any tag with different layers routes around them_.

**Component 3 completed and committed** (`feat(repo): add one-command containerized self-hosting`): one compose file/two modes (`full` profile), multi-stage node:22-slim Dockerfiles with `pnpm deploy --legacy` runtime layouts, one-shot `migrate` service gating api/worker on `service_completed_successfully`, dashboard + migrations + embedding model baked into the api image, model baked into the worker image. One real gap was caught during resume: the api image lacked the baked model even though `/search` embeds queries in-process (ADR-0018's air-gap promise) — fixed by mirroring the worker's `warm-model.ts`. Verified live end-to-end with throwaway credentials via a scratch `--env-file` (founder `.env` untouched): all services healthy on first boot, migrate idempotent, SPA/metrics/webhook-rejection smoke-checked. ADR-0020, `docs/self-hosting.md`, and the README/conventions/`.env.example` alignments shipped in the same change set; `pnpm verify` gated the commit.

## Session of 2026-07-19 (night) — M6 remaining scope: review rebuilt, approved, implemented; Docker fails a second time

The lost M6 design was **rebuilt from repository state** as a formal remaining-scope review (`docs/session-notes/m6-remaining-scope-review.md`) — committed as a draft _before_ approval, applying the morning's lesson (reviews die with sessions; the repo carries them now). Two committed breadcrumbs (the `demo:seed` script entry; the harness header naming `scripts/demo/seed.mjs`) pinned the seed design the crash had eaten. The founder approved D-M6-1…6 as recommended and resolved D-M6-7 as "keep the current merge policy; D11 unchanged during M6."

Implementation, in the approved order, each component verified then committed: **R1** the demo seeder (pipeline replay, deterministic GUIDs, locality guard; 6/6 scripted checks with scores matching the pinned decay arithmetic) + flaky-repo template — one engine truth surfaced and was written into the seed comments: _the event-driven recompute set only reassesses failing-or-non-healthy tests, so a divergence story must end on a failure or it scores 0_; **R2** dogfood wiring (five vitest configs emit JUnit; DevFlow's own parser verified against vitest's dialect directly; SHA-pinned `upload-artifact` v7.0.1 with `if: always()`); **R3** README/architecture M6 pass (deployment + observability sections, mermaid); **R4** CHANGELOG 0.1.0 + version bump + release checklist + demo storyboard. `pnpm e2e`: 21/21.

**Then Docker died again, differently:** mid-stranger-test, the compose plugin SIGBUS'd (its cross-distro mount broke), the `docker-desktop` distro stopped, and restarts looped on "backend server is already running" — root cause: the privileged Windows service `com.docker.service` is STOPPED and starting it needs elevation the session doesn't have. Recovery actions taken: standalone compose v5.3.1 installed user-scoped (`~/.docker/cli-plugins/`), full process sweep + relaunch (insufficient). **Honesty consequences, recorded rather than papered over:** R4's commit ran only the four Docker-free verify legs (diff = docs + version field; the same-day full verify + e2e stand as evidence), the stranger test is pending, and the release checklist row that had been drafted claiming it was corrected _before_ commit (NEVER-8). The v0.1.0 tag is gated on a healed Docker + the pre-tag gate (checklist step 0). The dev-log M6 entry is deliberately deferred to milestone completion.

## Session of 2026-07-20 — Docker root-caused and healed; the pre-tag gate passes; the stranger test earns its keep; M6 closed

**The Docker mystery resolved, anticlimactically:** after the founder freed ~124 GB on C:, the "Engine starting" hang turned out to be a _third_, unrelated failure — `wsl --shutdown` kills the VM but not the Windows-side `com.docker.backend.exe`, so a wedged backend held the pipes while every relaunch logged "backend server is already running" and deferred to it. Killing the stale processes and relaunching booted the engine in seconds. The fresh boot log then rewrote the whole diagnosis: yesterday's kmsg showed `Buffer I/O error … lost sync page write` at the _virtual-disk_ level — the sparse VHDX could not grow because **C: was exhausted**. The ext4 corruption, SIGBUS crashes and image-store damage were all downstream symptoms; ext4 journal recovery completed cleanly on the next boot, every image and volume survived, and no reset was needed. Ops lesson, standing: _when storage misbehaves, check capacity before integrity — and after `wsl --shutdown`, sweep the Windows-side Docker processes._

**The pre-tag gate, run in full:** `pnpm verify` green — 203 tests (queue 3, db 16, ai 21, worker 69, api 94), closing R4's recorded four-of-five-legs exception; `pnpm e2e` 21/21. Then the stranger test: fresh clone in a scratch directory, `docs/self-hosting.md` followed literally with throwaway credentials — full profile healthy on first boot, migrate exit 0, SPA served. **And it caught a real defect:** `pnpm demo:seed` after only `pnpm install` died with an opaque module-not-found + healthz timeout, because the spawned apps' workspace imports resolve through package exports to compiled `dist/` that a bare install doesn't have — a failure mode structurally invisible on any machine that has ever built. Fix (`4401817`, verify-gated): `assertBuilt()` preflight in the harness (fails with "run `pnpm build` first", negative-tested), build prerequisite added to the demo/e2e docs and the checklist. Re-run: seed 6/6 in the stranger stack, database populated per storyboard (12 runs, 51 results, 2 repos, 4 embeddings, correct verdicts); e2e re-run 21/21 with the changed harness.

**Machine-state notes:** the stranger stack was torn down; dev infra runs on fresh volumes (the old ones were removed for a clean stranger run — a pg_dump backup was taken first). The dev DB is migrated and seeded 6/6 but **unclaimed**: the founder's workspace lived in the old volume, so one login + `pnpm demo:seed` re-run attaches the demo data (the seeder prints exactly this). The e2e teardown prints a cosmetic crash dump from the spawned apps (the throwaway DB is force-dropped before they are killed) — after all checks pass; known, benign, left alone. The dev-log M6 entry, this entry, the checklist and the handoff were written and committed on the branch, completing the milestone's documentation set. **M6 is closed; push/PR/tag are the founder's.**

## Sessions of 2026-07-28 → 2026-08-01 — v0.1.x Stage 0 and Stage 1 T0–T6 (retro-recorded at T8)

Recorded at closeout per the Stage 1 plan (Stage 0's session also missed its handoff update — both retro-covered here). **Stage 0 (2026-07-28/29):** ADR-0022 (self-host-first) merged; the validation branch cut; the plan of record, prediction register (H1–H13) and this discipline ratified; T2's scaffolds ruled absorbed into the T1 commit. **T3–T4 (2026-07-30/31):** the GitHub App created against the guide walked literally — every UI divergence became a log entry and an in-guide fix; a first key exposure was rotated and revoked in-session; the §5 loop came up on real credentials; real deliveries persisted; the real Redeliver was absorbed as a duplicate (H13). **T5 (2026-08-01):** first real api.github.com pass — token dance, artifact listing, 302 zip download, parse, persist (H1–H5); real payloads normalized cleanly (H6). **T6 (2026-08-01):** live detection on `devflow-demo-flaky`; the neutral check created on the real Checks tab with the ID round-tripping to `flake_check_run_id` (H7 create); the plan's PATCH probe found resting on a queue-contract misreading, and the PATCH clause was instead confirmed via a founder-approved controlled eviction + redelivery. The same day, the branch was rewritten to strip Co-Authored-By trailers (hash mapping recorded in the log at T8). Full detail: [observation-log.md](../validation/observation-log.md).

## Session of 2026-08-06 — Stage 1 closeout: T6 on-the-PR, T7 login/claim/quarantine, T8 reconciliation

A restart-recovery session that finished the stage. **T6 closed:** after a full stack restart (the forgotten smee client re-learned as the fourth process), the re-run produced the single neutral DevFlow flake report on the real PR; DB `flake_check_run_id` = check-URL ID (92586551161); H7 fully confirmed. **T7:** real OAuth login through the Auth.js shim after two environment findings (embedded webviews drop the CSRF cookie; cookies are host-scoped so the dashboard must be browsed at `127.0.0.1`, never `localhost` — both now guide rules); the claim was learned to ride the _saved update_ redirect, not the settings-page visit (H10), with the live lifecycle event syncing during the Save (H11); the demo seed initially attached to an accidental junk workspace (first-created heuristic + a seeder check that could not fail — found, remedied, fixed at T8); the quarantine proposal approved in the real UI wrote `quarantine_records` row 1 — gate row 2. **T8:** H8 dispositioned `not-observable`, H12 confirmed stage-long; seeder check fixed; guide §3b and demo README amended; both checklist gate rows flipped ✅; hash mapping recorded and the one stale plan reference repaired; [validation-report.md](../validation/validation-report.md) written. Stage 1 complete except one clause — the live feed under real traffic, closed the next session — with Release + demo video unblocked (the portfolio-presentable milestone) and the Stage 2 soak proposed.

## Session of 2026-08-07 — the live-feed detective story; Stage 1 truly closed

The one clause left open at yesterday's closeout — the live feed under real traffic — took a full session to close, through four false negatives, each exonerated by recorded state rather than impressions: (1) the smee client was down (again) — zero deliveries, correctly quiet UI; (2) a retry predated the client's start; (3) a channel mismatch was suspected, and probes verified both the socket chain (authenticated Socket.IO clients through the real Vite ws proxy receiving a synthetic Redis-published event) and the tunnel leg (junk POST → smee → client → API 400-rejected at HMAC) — yet a fresh rerun still produced nothing; (4) the founder then verified the App's Webhook URL matched the channel character-for-character and that GitHub was creating **no** deliveries at all. Payload archaeology found the cause: **the T7 claim-flow Save had removed `devflow-demo-flaky` from the installation** at 2026-08-06 14:00:55 — the toggle's re-add half never happened, the claim's success masked it, and GitHub silently stopped creating deliveries for the repo. After the re-add, the first fair test passed on every layer at once: deliveries in, run row 31 processed, the worker's first-ever live events published (1/1/1), and the Runs page updating unrefreshed. Lessons made durable: guide §3b now warns that a toggled repository selection must be re-checked before the update-Save; the log's 2026-08-07 entries record the probe methodology; and twice during the hunt a "new run seen after refresh" turned out not to exist in the DB — the discipline of ruling on recorded state, not impressions, is what kept false evidence out of the record. Stage 1 closed with no open clauses.
