...
## D3. Auth: Auth.js (NextAuth) + Postgres, GitHub OAuth — Committed (ADR due when auth lands)
...
## D4. Real-time: Socket.IO self-hosted, Redis pub/sub for fan-out — Committed
...
ADRs expected in upcoming milestones: workspace multi-tenancy schema (M4), auth (M4), AI boundary formalization (M5).
